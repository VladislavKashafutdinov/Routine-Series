...
- Описать структуру `Config` с полем `Port` и чтением из `PORT` (дефолт `8080`)
- Создать chi-роутер с тремя middleware: `Recoverer` (защита от паники), логгер (метод, путь, статус, длительность), `Content-Type: application/json`
- Реализовать `cmd/server/main.go`: загрузка конфига → создание chi-роутера с middleware → запуск HTTP-сервера
- Настроить graceful shutdown: перехват SIGINT/SIGTERM, остановка сервера с таймаутом 30с
...
- Аннотировать `main.go` базовыми Swaggo-комментариями (название API, версия, описание, хост)
- Выполнить `swag init` для генерации `docs/swagger.json`
- Настроить отдачу Swagger UI по маршруту `/swagger/`
- Настроить отдачу `swagger.json` по маршруту `/api/v1/swagger.json`
...
## 6. Архивирование и восстановление активности
...
## 7. Управление параметрами серии
...
## 10. Награды: создание и просмотр
