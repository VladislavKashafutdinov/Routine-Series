...
- **Cross-domain operations get their own package** — operations spanning multiple domains (e.g. `dataimport/`, `health/`) are separate packages, not dumped into `app/`.
- **`api/` = front/back contract** — types and middleware that define the API contract (e.g. `ErrorResponse`, `ContentTypeJSON`).
- **`app/` = application infrastructure** — how the app works internally, not the API contract (e.g. `Logger`).
- **Config lives where it's used** — `DATABASE_URL` stays inside `dbpool/`, `PORT` stays in `main.go`. No global config package.
- **Per-package `Handlers` struct** — each package exposes a `Handlers{Pool}` struct with methods, instead of closures or a global App struct. Dependencies are explicit.
