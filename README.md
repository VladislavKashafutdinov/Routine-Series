...
`Spinner` — общий индикатор ожидания: показывается в интерактивных элементах (кнопках/ячейках), отправляющих мутирующие запросы, пока запрос выполняется; сами элементы на это время неактивны (disabled).
