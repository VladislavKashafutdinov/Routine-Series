...
### 2026-09-04 — Ручное обновление данных

- Кнопка обновления на десктопе (левый верхний угол), pull-to-refresh на мобильных
- Кнопка видна и на мобильных (pull-to-refresh работает не везде)

### 2026-09-04 — Переезд БД на RelaxDev

- БД переехала с Render/Neon на внутреннюю БД RelaxDev без потери данных, документация обновлена

### 2026-09-03 — Мобильная версия: тап с первого раза

- `touch-action: manipulation` — кнопки «Отметить» нажимаются с первого тапа (без перевода фокуса)

### 2026-09-02 — Спиннеры на мутирующих запросах

- Все интерактивные элементы, отправляющие мутирующие запросы в API, показывают спиннер и блокируются до ответа (компонент `Spinner`, 13 компонентов)

### 2026-09-01 — Обработка ошибок и UX-доработки

- Обработка ошибок API на фронте: автоскрывающийся toast-попап с user-friendly сообщением для мутирующих запросов + экран ошибки загрузки с кнопкой повтора (вместо пустого интерфейса)
- Вкладка «Награды»: баннеры заработанных неначисленных наград перенесены с главной страницы, кнопка «Начислить всё», бейджик при неначисленных наградах
- AddActivity выглядит как ActivityCard: карточка с «+», добавление через модалку (название, параметры начальной seriesDefinition)
- Логирование причины 409 (конфликт) при удалении активности; попутно `/api/v1/data` отдаёт activity_id без фильтра дат
- Раскрывающееся меню в правом верхнем углу: выход, выбор языка, экспорт/импорт, виртуальное «сегодня», переход в архив

### 2026-09-01 — Единый запрос загрузки данных

- `GET /api/v1/data` — все данные одним запросом вместо N+1 параллельных запросов на каждую активность

### 2026-08-31 — Обработка таймаутов GET-запросов

- Бэк отдаёт 504 при истечении дедлайна, фронт ретраит с политикой: лимит попыток + бэкофф (1с/2с) + случайный jitter

### 2026-08-30 — Расследование зависаний БД на RelaxDev

- ConnectTimeout и HealthCheckPeriod в пул pgx
- Логирование причины 500 в RequireAuth, уровни логов (5xx ERROR, 4xx WARNING)
- WriteTimeout поднят — клиент получает честный 500 вместо бесконечного ретрая
- pgx-трейсер (коннекты, медленные запросы, acquire) + PingTimeout 5s
- RequestTimeout 15s на защищённые маршруты (+ фикс порядка middleware: RequestTimeout снаружи RequireAuth)
