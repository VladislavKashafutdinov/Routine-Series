...
- Оптимизация загрузки: после мутаций данные обновляются точечно из ответов API (копия в памяти в `ActivitiesContext`), полная перезагрузка с сервера больше не происходит
- Добавлен оверлей со спиннером (`LoadingOverlay`) при полной загрузке данных по API — блокирует взаимодействие во время загрузки
- Debug-фикс: фронт подгружает `.env` независимо от каталога запуска (vite.config.ts)

## 2026-08-29

- В деплой GitHub Pages добавлен `workflow_dispatch` (ручной запуск)

## 2026-08-30

- Расследование зависаний БД на RelaxDev:
  ...
  - pgx-трейсер коннектов и медленных запросов
  - PingTimeout 5s + acquire-трейсер

## 2026-08-31

- RequestTimeout 15s на защищённые маршруты
- Фикс порядка middleware: RequestTimeout снаружи RequireAuth
- Обработка таймаутов GET: бэк отдаёт 504 при истечении дедлайна, фронт ретраит с политикой (лимит попыток + бэкофф 1с/2с + случайный jitter)

## 2026-09-01

- Единый запрос загрузки данных: `GET /api/v1/data` вместо N+1 параллельных запросов на каждую активность
- Обработка ошибок API на фронте: автоскрывающийся toast-попап для мутирующих запросов, экран ошибки загрузки с кнопкой повтора
- Вкладка «Награды»: баннеры неначисленных наград перенесены с главной страницы, кнопка «Начислить всё», бейджик на вкладке
- AddActivity оформлен как карточка с «+» после секций, добавление активности через модалку
- Логирование причины 409 при удалении активности; попутно `/api/v1/data` отдаёт activity_id без фильтра дат
- Раскрывающееся меню в правом верхнем углу: выход, язык, экспорт/импорт, TimeTravel (виртуальное «сегодня»), переход в архив
